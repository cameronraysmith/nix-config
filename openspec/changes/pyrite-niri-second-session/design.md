...
**Runtime clarification (2026-09-09): cameron's remembered session is now niri.** GNOME remains the default for someone with no recorded choice, not for cameron after choosing niri. The next login defaults to niri unless the cog is used to pick GNOME: fallback is now one cog-click away, not the session obtained without a new selection. This is the explicit never-chosen/remembered-choice distinction in the delta spec's scenarios, not a guarantee of unconditional GNOME login. GDM 50.1 reads AccountsService's session (`daemon/gdm-session-settings.c:285-309`), prioritizes it over fallback (`daemon/gdm-session.c:687-694`), and tries GNOME first only in fallback (`:621-625`). Observed persisted file and attribution correction: `verify.md` §9.1. GNOME's new interactive fallback login remains unobserved.
...
**Delivered-behavior correction (2026-09-09).** The authorized switch wrote the correct file but did not reload running logind; the orchestrator observed D-Bus `HandlePowerKey=poweroff` until a manual reload at 19:02:28 UTC. Deployment does not fully take effect without that reload or a reboot. Task 6.5 now makes the post-deploy reload and live D-Bus check explicit, before any power-key test; no reboot is authorized here. The effective pin's module sets `reloadIfChanged` but leaves its config-file `restartTriggers` commented out (`logind.nix:67-73`). File contents alone are not proof of daemon state. `verify.md` §9.2.
...
The configuration does not rewrite AccountsService, but **GDM does persist an operator's selection**; choosing GNOME is therefore the operational rollback for cameron, and also records GNOME for later logins. “No state to unwind” above refers to configuration-driven migration, not absence of per-user state.
...
**Runtime qualification (2026-09-09).** Both backend services and the frontend acquired their D-Bus names at 19:04:59 and remained up until the niri compositor exited at 19:07:31. The GNOME/GTK status-1 failures are compositor-disconnect teardown noise, not evidence that no backend ever started. Delivered `niri-portals.conf` and Nautilus's activatable service agree with the module. However, “sufficient” above is a source/wiring conclusion, **not** an observed successful file dialog or screencast transaction. A separate GNOME GlobalShortcutsProvider warning occurred during startup. See `verify.md` §9.3 and **B-portal** below; no source repair or slice-A portal task is added.
...
## Slice-B follow-up references (known limitations, not slice-A tasks)

- **B-cursor:** install/select a real cursor theme when assembling slice B. Delivered config uses `xcursor-theme "default"`; niri reports `error loading xcursor default@48: no default icon`. Cosmetic lookup failure; the operator reports no theme installed. Evidence and limits: `verify.md` §9.4.
- **B-xwayland:** provide `xwayland-satellite` on the session PATH if slice B requires X11 applications. niri itself failed to spawn it and disabled integration; OQ1 remains the accepted limitation. `verify.md` §9.4.
- **B-portal:** retain the working backend wiring and exercise actual FileChooser and ScreenCast requests during slice-B integration. Investigate the observed GNOME GlobalShortcutsProvider `Invalid service client type` warning and any dialog effects; no file-dialog/screencast failure was demonstrated. Review teardown failure-state hygiene separately from functional startup. `verify.md` §9.3. These are local follow-up references, not claimed external issue IDs.

**Historical citation correction:** `logs/niri-slice-a-apply-phase1b.md:193-220` incorrectly superseded the `85f6261` source reads by looking up the node merely named `nixpkgs`. Root actually follows `nixpkgs_9 → 85f6261`; `logs/niri-slice-a-ledger.md:18` repeats the same metadata error. This artifact and `tasks.md` correct the attribution; those historical logs are not rewritten. Evidence: `logs/niri-slice-a-verify.md` §4; effective-pin logind reread: `logs/niri-final-portal-context-20260909.log:747-852`.

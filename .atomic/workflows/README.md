...
`deploy-omnigent.ts` runs S0–S5 as create/reuse → implement → scope → land → gate-sandbox → diff → review; bounded repairs repeat into the same change.
Landing uses only attributed paths with `jj squash --from @ --into <change> --use-destination-message --keep-emptied -- <paths>`; ownership baselines, preexisting-path protection, topology and foreign-path human decisions remain enforced.
Empty attribution skips squash only for an already-populated allowed change; gate batches use detached temporary Git worktrees at the exported SHA and clean them in `finally`, never jj workspaces.
GrepAssert and Command gates run there; Nix gates use `git+file://<absolute-repo>?ref=<chain_name>&rev=<sha>`; Linear readback uses the absolute primary script with the sandbox root, Python/PyYAML and stored workspace credentials, ignoring inherited `LINEAR_API_KEY`.
Reviews receive the contract, SHA-bearing receipts and change diff; SHA checks before/after review and before repair landing reject reviewed-commit drift, not unrelated edits, but are not concurrency locks.
An unchanged repair after `changes_requested` reruns all gates once and blocks if review or gates still fail.
Every stage requests only `openai-codex/gpt-6-astra`: high for implementation/repair/tracking, max for review, medium for reporting; missing/off-policy actual model/thinking metadata blocks.
Required `splice_after` names the seed parent; other inputs are `chain_name` (`omnigent-magnetite`), `start_at_slice` (0; range 0–6), `verified_changes` ([]), `max_repair_attempts` (2), `build_timeout_minutes` (60), `deploy` (true), and `linear_team` (`CAM`).
Resume revalidates topology/path sets and preceding slices' gates at their own SHAs; S5 is runtime-environment and deployment is phase 6.
S0 recovery accepts only seed → join or seed → allowed-path (possibly empty) S0 → join with empty `verified_changes`; it records reuse and skips tracking only when the proposal is observed in `@`.
S4 derives one shared ACP definition for the server's read-only store `OMNIGENT_CONFIG_HOME` and runner's runtime-written config, merging declared settings without clobbering `host.host_id`; gates read the server YAML, rehearse the merge and build magnetite remotely.
S5 removes late Claude TMPDIR/TMPPREFIX overrides, including inherited GLM/Cerebras copies, and requires bare Python in the runner module's runtime packages, not host `extraPackages`.
Its objective/commit justification records the 2.1.45/2.1.47 sandbox fixes and auxiliary-temp cost; the plan amendment declares harness-visible environment once at the service boundary and records the direnv protected-root hazard with remedies deferred to the operator.
S5 gates inspect evaluated settings and rendered JSON for both users and all three profiles, Python on the unit path, plan justification and the remote magnetite closure.
Secret generation remains restricted to the two Omnigent generator directories and routes to S3; tfstate remains on S2; deployment resolves the tip SHA after topology checks.
Deployment requires the exported bookmark to equal that SHA (one export retry); pinned `terraform.config` and argument-forwarding `terraform.terraform` use shared encrypted state, never the non-forwarding default app.
The saved DNS plan permits exactly one create of `cloudflare_dns_record` named `omni.scientistexperience.net`; confirmation shows its summary, apply verifies its hash, and secret-bearing plan/JSON files stay mode 600 under evidence.
`clan machines update magnetite --flake "$SOURCE"` records URL/SHA and requires changed `/run/current-system`; server, catalog and read-only `probe-claude-hook-env` checks precede the wizard, and runner-settings/online probes follow it.
The catalog probe reads only agent name/command fields; the Claude probe rejects deployed `~cameron/.claude/settings.json` containing `/tmp/claude`, without printing settings.
Human checklist items include the Atomic dropdown/session, structured Claude response (not terminal-only) and Files populated after Resume; `failed` blocks, `not tested` leaves acceptance incomplete, and all-passed is only `human_attested`.
Evidence lives under `.atomic/workflows/runs/deploy-omnigent/<chain_name>/<run-key>/`; stop-for-replan preserves verified changes, landed-but-unverified content and unlanded edits.
Run `node .atomic/workflows/omnigent/check.mjs` for strict typing, negative fixtures, mocked execution, ShellCheck and Nix parsing, not evaluation/builds/deployment/jj mutations; the controlling session owns registry reload and launch, and live acceptance remains untested.
...
G1 requires exactly three subscribable event names: `check_run`, `pull_request`, `status`. GitHub automatically delivers `installation` and `installation_repositories` to every App; they cannot be subscribed to and are never required in the API `events` array. `app.json` records the automatic-delivery contract and any extra reported subscriptions; extras do not block, but a missing required subscription or any permission mismatch does. The existing `sciexp-gitea-mq` registration needs no change. Task 1.4 still requires nixbot's unchanged events `["check_run","check_suite","pull_request","push"]`.
...
Before any credential generation, `generateVars` observes `CLAN_NO_COMMIT=1 clan vars list magnetite`, parsing only `gitea-mq-github-app-secret-key/key.pem` and `gitea-mq-github-webhook-secret/secret`. Other rows are ignored regardless of format or public values. A target's `********` or any non-empty value means `already-present`; only an explicit `<not set>` webhook row permits generation with `--generator gitea-mq-github-webhook-secret --no-regenerate`, followed by a fresh listing. Missing, duplicate, empty/malformed target rows and nonzero exits block. An unset App PEM blocks with the operator instruction `CLAN_NO_COMMIT=1 clan vars set magnetite gitea-mq-github-app-secret-key/key.pem`; the workflow never generates or sets it. Listing output is bounded in memory and projected to target statuses before any log/receipt write; non-target rows, all real values, and raw stderr are withheld, including failure paths. Existing webhook secrets retain an `already-present` observation bound to the list receipt.
The post-effect backstop compares `@`'s change identity and ordered parent commit identities, the exact `rollup-landing` revision, and only the protected join/seed/tip/owned-chain revisions and their membership in join ancestry—not every foreign ancestor. It deliberately ignores `@`'s content commit ID, which may change when the shared working copy is snapshotted. Protected missing/divergent identities block with their identity and candidate commit IDs (`[]` for absence). Foreign divergence, whether already present or appearing during the Clan window, is non-blocking `foreignDivergent: [{ changeId, candidates }]` evidence. New/re-written non-`@` commits are inspected at immutable commit IDs: any modified path under `vars/per-machine/magnetite/**` blocks; all others are recorded as `foreignChanges` (change ID, commit ID, paths) without blocking, including unrelated siblings. Unrelated disk edits are `foreignDrift`; magnetite vars disk changes are allowed only in the webhook generator directory during successful missing-secret generation. No global Git/JJ commit-set equality remains. Checks run in `finally`, including on list/generation failures; they are a backstop, not the primary prevention mechanism.
Revision lookups use explicit `change_id(<id>)` sets or immutable commit IDs. Singleton checks inspect all visible candidates even when `@`, a bookmark or a graph edge selected just one version. `ids` retains multiple results as a set observation; it does not impose protected uniqueness on foreign changes. Development health no longer rejects a foreign `(divergent)` annotation. Chain edges are scoped to the expected owned child, not a global sole-child query; new routing constrains insertion with both `-A <tip>` and `-B <join>` so foreign siblings are not rebased. Shared deployment helpers receive only locally verified commit IDs.
...
Credential-generation regressions run separately with `node .atomic/workflows/gitea-mq/check.mjs --vars-only` (real controller, closed command/filesystem/VCS ports): present/missing/unparseable listings, receipt binding, foreign commits/disk drift/divergence (existing and newly divergent), protected divergent/absent identities with candidate diagnostics, new magnetite-vars commits, protected topology movement, and failures. `--vcs-only` tests the real revision/health/diff helpers with only their process port mocked. The full check includes the same cases.

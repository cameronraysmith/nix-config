...
The post-review `stable-*` guard is a return-mode gate inside the S1 bound: drift invalidates the attempt, diagnoses/repairs and requires another fresh review. `verify-structural-input-b*-a*` likewise runs inside a bounded change-report repair loop before deterministic report rendering; repeated structural failures reach G3 and then stop without positive outputs.

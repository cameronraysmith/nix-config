...
For adoption, `forge-pre-unchanged` never compares against the already-implemented working copy: preflight resolves the current `rollup-landing` commit through Omnigent's shared source resolver, rejects a source already containing the S1 aspect, and evaluates `ExecStartPre` from `git+file://<repo>?ref=rollup-landing&rev=<sha>`. The S1 gate evaluates the identical `--apply` projection on the candidate and records source provenance, both evaluation receipt paths/values, and the comparison. Missing immutable evaluation yields `NotRun(reason)` and leaves task 5.3 unticked; a mismatch fails the gate. Non-adoption retains its genuinely pre-S1 working-copy baseline with explicit provenance. Legacy baseline receipts without provenance cannot verify this observable.
